...
def flood_filter(row):
    ...
    for keyword in POSITIVE_KEYWORDS:
        keyword_count = len(
            re.findall(r"\b%s\b" % keyword.lower(), tender_slug.lower())
        )
    ...
    return (
        str(is_flood_tender),
        str(positive_keywords_dict),
        str(negative_keywords_dict),
    )
...
for csv in csvs:
    ...
    input_df.loc[:, "positive_keywords_dict"] = [
        var[1] for var in list(flood_filter_tuples)
    ]
    input_df.loc[:, "negative_keywords_dict"] = [
        var[2] for var in list(flood_filter_tuples)
    ]
    ...
    for index, row in idea_frm_tenders_df.iterrows():
        ...
        is_present = [
            len(re.findall(r"\b%s\b" % kw.lower(), tender_slug.lower()))
            for kw in EROSION_KEYWORDS
        ]
        if sum(is_present) > 0:
    ...
    for index, row in idea_frm_tenders_df.iterrows():
        ...
        is_present = [
            len(re.findall(r"\b%s\b" % kw.lower(), tender_slug.lower()))
            for kw in ROADS_BRIDGES_EMBANKMENTS_KEYWORDS
        ]
        if sum(is_present) > 0:
    ...
    PREPAREDNESS_KEYWORDS = [
        ...
        "aapda mitra volunteer",
        "district emergency stockpile",
        "search light",
        "life buoys",
        "boat ambulances",
        "boat ambulance",
    ...
    for index, row in idea_frm_tenders_df.iterrows():
        ...
        repair_restoration_dict = populate_keyword_dict(
            REPAIR_RESTORATION_IMPROVEMENTS_KEYWORDS
        )
        ...
        if response_type == "Immediate Measures":
            sub_head_dict = {
                k: v for k, v in immedidate_measures_dict.items() if v is not False
            }
            idea_frm_tenders_df.loc[index, "Flood Response - Subhead"] = str(
                sub_head_dict
            )
        elif response_type == "Repair and Restoration":
            sub_head_dict = {
                k: v for k, v in repair_restoration_dict.items() if v is not False
            }
            idea_frm_tenders_df.loc[index, "Flood Response - Subhead"] = str(
                sub_head_dict
            )
            ...
            sub_head_dict = {
                k: v for k, v in preparedness_measures_dict.items() if v is not False
            }
            idea_frm_tenders_df.loc[index, "Flood Response - Subhead"] = str(
                sub_head_dict
            )
    ...
    idea_frm_tenders_df.to_csv(
        os.getcwd()
        + "/Sources/TENDERS/scripts/scraper/assam/data/flood_tenders/"
        + filename,
        encoding="utf-8",
        index=False,
    )
...
data_path = os.getcwd() + "/Sources/TENDERS/scripts/scraper/assam/data/"
