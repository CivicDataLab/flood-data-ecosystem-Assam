...
ASSAM_RCS = gpd.read_file(
    os.getcwd() + "/Maps/Assam_Revenue_Circles/assam_revenue_circle_nov2022.geojson",
    driver="GeoJSON",
)
...
for FOCUS_DISTRICT in tqdm(ASSAM_VILLAGES.district_2.unique()):
    ...
    for index, row in ASSAM_VILLAGES[
        ASSAM_VILLAGES.district_2 == FOCUS_DISTRICT
    ].iterrows():
        if row["VILNAM_SOI"]:
            ...
            FOCUSDIST_village_dict[row["VILNAM_SOI"]] = {
    ...
    idea_frm_tenders_df_FOCUSDISTRICT = idea_frm_tenders_df[
        idea_frm_tenders_df["DISTRICT_FINALISED"] == FOCUS_DISTRICT
    ]
    for idx, row in idea_frm_tenders_df_FOCUSDISTRICT.iterrows():
        ...
        for revenue_circle in FOCUSDIST_revcircles:
            ...
            if re.findall(
                r"\b%s\b" % revenue_circle_search.strip(), tender_slug.lower()
            ):
        ...
        for revenue_circle in FOCUSDIST_revcircles:
            ...
            revenue_circle_search = re.sub(pattern, " ", revenue_circle_search)
            if re.findall(
                r"\b%s\b" % revenue_circle_search.strip(), row["location"].lower()
            ):
        ...
        idea_frm_tenders_df_FOCUSDISTRICT.loc[idx, "tender_villages"] = str(
            tender_villages
        )[1:-1]
        idea_frm_tenders_df_FOCUSDISTRICT.loc[idx, "tender_block"] = tender_block
        idea_frm_tenders_df_FOCUSDISTRICT.loc[idx, "tender_subdistrict"] = (
            tender_subdistrict
        )
        idea_frm_tenders_df_FOCUSDISTRICT.loc[idx, "tender_revenueci"] = (
            tender_revenueci
        )
        idea_frm_tenders_df_FOCUSDISTRICT.loc[idx, "tender_revenueci_location"] = (
            tender_revenueci_location
        )
...
MASTER_DFs.append(
    idea_frm_tenders_df[idea_frm_tenders_df["DISTRICT_FINALISED"] == "NA"]
)
MASTER_DFs.append(
    idea_frm_tenders_df[idea_frm_tenders_df["DISTRICT_FINALISED"] == "CONFLICT"]
)
...
for idx, row in MASTER_DF.iterrows():
    ...
    if row["HQ_flag"] == False:
        MASTER_DF.loc[idx, "REVENUE_CIRCLE_FINALISED"] = row[
            "tender_revenueci_location"
        ]
...
MASTER_DF.to_csv(
    os.getcwd()
    + "/Sources/TENDERS/scripts/scraper/assam/data/floodtenders_RCgeotagged.csv"
)
