...
os.makedirs(
    os.getcwd()
    + "/Sources/TENDERS/scripts/scraper/odisha/scraped_recent_tenders/"
    + folder,
    exist_ok=True,
)
# except:
...
os.makedirs(
    os.getcwd()
    + "/Sources/TENDERS/scripts/scraper/odisha/scraped_recent_tenders/concatinated_csvs",
    exist_ok=True,
)
# except:
#     pass

url = "https://tendersodisha.gov.in/nicgep/app?page=WebTenderStatusLists&service=page"  # "https://assamtenders.gov.in/nicgep/app?page=WebTenderStatusLists&service=page"
options = Options()
...
os.chdir(os.getcwd() + "/Sources/TENDERS/scripts/scraper/odisha/scraped_recent_tenders")
dict_tables_type = {
    "Bids List": "Vertical",
...
def captcha_input(xpath_image, xpath_input_text):
    print("Enter captcha func")
    captcha_text = captcha(browser, xpath_image)
    print("Captch text: ", captcha_text)
    # time.sleep(10)
    # pdb.set_trace()
    captcha_input_element = SeleniumScrappingUtils.get_page_element(
        browser, xpath_input_text
    )
    ...
    if len(invalid_string) == 0:
        ...
        while "Invalid Captcha!" in invalid_string[0].text:
            ...
            SeleniumScrappingUtils.input_text_box(
                browser, captcha_input_element, captcha_text
            )
            time.sleep(3)
...
for tender_status_id in range(6, 7):
    ...
    SeleniumScrappingUtils.select_drop_down(
        browser, '//*[@id="tenderStatus"]', tender_status_id
    )  # 3
    ...
    SeleniumScrappingUtils.select_drop_down(
        browser,
        '//*[@id="Body"]/div[2]/div[1]/table/tbody/tr/td[2]/select',
        value=month_start,
    )
    # Select year
    SeleniumScrappingUtils.select_drop_down(
        browser, '//*[@id="Body"]/div[2]/div[1]/table/tbody/tr/td[3]/select', value=year
    )
    # Select Date
    ...
    SeleniumScrappingUtils.select_drop_down(
        browser,
        '//*[@id="Body"]/div[3]/div[1]/table/tbody/tr/td[2]/select',
        value=month_end,
    )
    # Select year
    SeleniumScrappingUtils.select_drop_down(
        browser, '//*[@id="Body"]/div[3]/div[1]/table/tbody/tr/td[3]/select', value=year
    )
    # Select Date
    ...
    # print("Sleeping before entering captcha")
    # time.sleep(10)
    # break captcha
    ...
    def scrape_view_more_details(browser, tender_id):
            ...
            try:
                ...
                elem = wait.until(
                    EC.presence_of_element_located(
                        (
                            By.XPATH,
                            "/html/body/table/tbody/tr/td/table/tbody/tr[4]/td/table/tbody/tr/td/table/tbody/tr/td/table/tbody/tr[2]/td/table/tbody/tr[1]/td",
                        )
                    )
                )
                ...
                pass
        tables = SeleniumScrappingUtils.get_multiple_page_elements(
            browser,
            "/html/body/table/tbody/tr/td/table/tbody/tr[4]/td/table/tbody/tr/td/table/tbody/tr/td/table",
        )[0].find_elements(By.CSS_SELECTOR, "table")
        dict_table_section_head = {}
        for table_section_elements in tables:
            try:
                dict_table_section_head[
                    table_section_elements.find_element(
        ...
        for index, (keys, values) in enumerate(dict_table_section_head.items()):
            ...
            #     SeleniumScrappingUtils.extract_horizontal_table(values,tender_id +"_"+"Work_Item"+"_" + str(index),1)
                ...
                keys.startswith("Cover Details")
                or keys == "Latest Corrigendum List"
                or keys.startswith("Other")
            ):
                SeleniumScrappingUtils.extract_vertical_table(
                    values, tender_id + "_" + keys + "_" + str(index), 1
                )
            elif keys == "Payment Instruments":
                table_section = values.find_element(By.CSS_SELECTOR, "table")
                SeleniumScrappingUtils.extract_vertical_table(
                    table_section, tender_id + "_" + keys + "_" + str(index), 1
                )
            else:
                SeleniumScrappingUtils.extract_horizontal_table(
                    values, tender_id + "_" + keys + "_" + str(index), 1
                )
        path_to_save = "concatinated_csvs/"
        SeleniumScrappingUtils.concatinate_csvs(
            path_to_save, tender_id, dict_tender_status[tender_status_id]
        )
    ...
    def scrape_view_stage_summary(browser, tender_id, dict_tables_type):
        ...
        SeleniumScrappingUtils.get_page_element(
            browser, '//*[@id="DirectLink_0"]'
        ).click()
        # time.sleep(3)
        ...
        for index, name in enumerate(sections):
            if index == 0:
                SeleniumScrappingUtils.extract_horizontal_table(
                    name, "Org_" + tender_id, 0
                )
                continue
            header_name = name.find_element(By.CLASS_NAME, "section_head").text.strip()
            if (header_name in list_of_dict_tables_type) & (
                dict_tables_type[header_name] == "Vertical"
            ):
                SeleniumScrappingUtils.extract_vertical_table(
                    name, header_name + "_" + tender_id, 1
                )
            else:
                SeleniumScrappingUtils.extract_horizontal_table(
                    name, header_name + "_" + tender_id, 1
        ...
        try:
            SeleniumScrappingUtils.concatinate_csvs(
                path_to_save,
                "summary" + "_" + tender_id,
                dict_tender_status[tender_status_id],
            )
        except:
            pass
    ...
    def get_table_links(browser, table_xpath):
        ...
        try:
            next_page_link = table.find_elements("xpath", '//*[@id="loadNext"]')[
                0
            ].get_attribute("href")
        except:
            next_page_link = ""
    ...
    if __name__ == "__main__":
        tender_ids_list = []
        table, links, next_page_link, tender_ids = get_table_links(
            browser, '//*[@id="tabList"]'
        )
