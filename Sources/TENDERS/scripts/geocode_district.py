...
idea_frm_tenders_df = pd.read_csv(
    os.getcwd() + "/Sources/TENDERS/scripts/scraper/assam/data/flood_tenders_all.csv"
)

# ASSAM_VILLAGES = gpd.read_file(os.getcwd()+'/Maps/assam_village_complete_with_revenueCircle_district_35_oct2022.geojson',
#                              driver='GeoJSON')
ASSAM_VILLAGES = pd.read_csv(
    os.getcwd() + "/Maps/ASSAM_VILLAGES_MASTER.csv", encoding="utf-8"
).dropna()

# Clean village names
assam_villages = ASSAM_VILLAGES["VILNAM_SOI"]
village_duplicates_df = ASSAM_VILLAGES[
    assam_villages.isin(assam_villages[assam_villages.duplicated()])
].sort_values("VILNAM_SOI")
...
for idx, row in idea_frm_tenders_df.iterrows():
    ...
    if max(scores) > 0.8:
        locations.append(
            ASSAM_VILLAGES.revenue_ci.dropna().unique()[scores.index(max(scores))]
        )
    else:
...
problematic_rev_circles = rev_circles[
    rev_circles.duplicated(["revenue_ci"], keep=False)
].sort_values("revenue_ci")

sdts = ASSAM_VILLAGES[["sdtname_2", "district_2"]].drop_duplicates().dropna()
# These revenue circles are across multiple districts
problematic_sdts = sdts[sdts.duplicated(["sdtname_2"], keep=False)].sort_values(
    "sdtname_2"
)
...
assam_revenue_circles_dict = (
    ASSAM_VILLAGES[["revenue_ci", "district_2"]]
    .dropna()
    .drop_duplicates()
    .drop_duplicates(["revenue_ci"], keep=False)
    .set_index("revenue_ci")
    .to_dict(orient="index")
)
assam_subdist_dict = (
    ASSAM_VILLAGES[["sdtname_2", "district_2"]]
    .dropna()
    .drop_duplicates()
    .drop_duplicates(["sdtname_2"], keep=False)
    .set_index("sdtname_2")
    .to_dict(orient="index")
)
assam_blocks_dict = (
    ASSAM_VILLAGES[["block_name", "district_2"]]
    .dropna()
    .drop_duplicates()
    .drop_duplicates(["block_name"], keep=False)
    .set_index("block_name")
    .to_dict(orient="index")
)
assam_villages_dict = (
    ASSAM_VILLAGES[["VILNAM_SOI", "district_2"]]
    .drop_duplicates(["VILNAM_SOI"], keep=False)
    .set_index("VILNAM_SOI")
    .to_dict(orient="index")
)
...
problematic_rev_circlesUPPERCASE = (
    []
)  # Empty after forcefitted. #[rc.upper().strip() for rc in problematic_rev_circles.revenue_ci.unique()]
problematic_sdtsUPPERCASE = [
    sdt.upper().strip() for sdt in problematic_sdts.sdtname_2.unique()
]
assam_villages = list(
    set(assam_villages_dict.keys())
    - set(problematic_rev_circlesUPPERCASE)
    - set(problematic_sdtsUPPERCASE)
)
assam_blocks = list(
    set(assam_blocks_dict.keys())
    - set(problematic_rev_circlesUPPERCASE)
    - set(problematic_sdtsUPPERCASE)
)
assam_districts = list(
    set(ASSAM_VILLAGES.district_2.dropna()) - set(["KAMRUP", "KAMRUP METRO"])
)

assam_revenue_circles = list(
    set(assam_revenue_circles_dict.keys())
)  # -set(problematic_rev_circles.revenue_ci.unique())-set(problematic_sdts.sdtname_2.unique()))
assam_sub_districts = list(
    set(assam_subdist_dict.keys())
    - set(problematic_rev_circles.revenue_ci.unique())
    - set(problematic_sdts.sdtname_2.unique())
)
...
three_letter_distirct_identifiers_dict = {
    ...
    "badarpur": "KARIMGANJ",
    "gmda": "KAMRUP METRO",
    "ghy east": "KAMRUP METRO",
    "ghy.east": "KAMRUP METRO",
    "ghy. east": "KAMRUP METRO",
    "ghyeast": "KAMRUP METRO",
    "g.east": "KAMRUP METRO",
    "ghy east": "KAMRUP METRO",
    "ghy west": "KAMRUP METRO",
    "ge": "KAMRUP METRO",
    "ghy.west": "KAMRUP METRO",
    "ghy. west": "KAMRUP METRO",
    "ghywest": "KAMRUP METRO",
    "kok": "KOKRAJHAR",
    "rangia": "KAMRUP",
    "gdd": "KAMRUP METRO",
}
...
for idx, row in idea_frm_tenders_df.iterrows():
    ...
    if district_identifier in three_letter_distirct_identifiers_dict:
        idea_frm_tenders_df.loc[idx, "tender_district_externalReference"] = (
            three_letter_distirct_identifiers_dict[district_identifier]
        )
...
for idx, row in idea_frm_tenders_df.iterrows():
    ...
    for revenue_circle in assam_revenue_circles:
        if re.findall(r"\b%s\b" % revenue_circle.lower().strip(), tender_slug.lower()):
            idea_frm_tenders_df.loc[idx, "tender_district_externalReference"] = (
                assam_revenue_circles_dict[revenue_circle]["district_2"]
            )
            break
...
for idx, row in idea_frm_tenders_df.iterrows():
    ...
    for sub_district in assam_sub_districts:
        if re.findall(r"\b%s\b" % sub_district.lower(), tender_slug.lower()):
            idea_frm_tenders_df.loc[idx, "tender_district_externalReference"] = (
                assam_subdist_dict[sub_district]["district_2"]
            )
            break
...
for idx, row in idea_frm_tenders_df.iterrows():
    ...
    for revenue_circle in assam_revenue_circles:
        if re.findall(r"\b%s\b" % revenue_circle.lower().strip(), tender_slug.lower()):
            ...
            idea_frm_tenders_df.loc[idx, "tender_district_title_description"] = (
                assam_revenue_circles_dict[revenue_circle]["district_2"]
            )
            break
...
for idx, row in idea_frm_tenders_df.iterrows():
    ...
    for revenue_circle in assam_revenue_circles:
        if re.findall(r"\b%s\b" % revenue_circle.lower().strip(), tender_slug.lower()):
            idea_frm_tenders_df.loc[idx, "tender_district_location"] = (
                assam_revenue_circles_dict[revenue_circle]["district_2"]
            )
            break
...
idea_frm_tenders_df.to_csv(
    os.getcwd()
    + "/Sources/TENDERS/scripts/scraper/assam/data/floodtenders_districtgeotagged.csv",
    index=False,
)

print("Total number of flood related tenders: ", idea_frm_tenders_df.shape[0])
print(
    "Number of tenders whose district could not be geo-tagged: ",
    idea_frm_tenders_df[idea_frm_tenders_df["DISTRICT_FINALISED"] == "NA"].shape[0],
)
print(
    "Number of tenders whose district identification is a CONFLICT: ",
    idea_frm_tenders_df[idea_frm_tenders_df["DISTRICT_FINALISED"] == "CONFLICT"].shape[
        0
    ],
)
