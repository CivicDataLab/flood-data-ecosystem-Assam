...
sdrf_df = pd.read_csv(os.getcwd()+r'IIDS-DRR-Assam\Sources\SDRF\data\flood_expenditure.csv')

#ASSAM_VILLAGES = gpd.read_file(os.getcwd()+'/Maps/assam_village_complete_with_revenueCircle_district_35_oct2022.geojson',
 #                              driver='GeoJSON')
ASSAM_VILLAGES = pd.read_csv(os.getcwd()+r'/IDS-DRR-Assam/Maps/ASSAM_VILLAGES_MASTER.csv', encoding='utf-8').dropna()
...
ASSAM_VILLAGES.revenue_ci = ASSAM_VILLAGES.revenue_ci.str.replace(r'\(Pt\)','')
ASSAM_VILLAGES.revenue_ci = ASSAM_VILLAGES.revenue_ci.str.replace(r'\(Pt-I\)','')
ASSAM_VILLAGES.revenue_ci = ASSAM_VILLAGES.revenue_ci.str.replace(r'\(Pt-II\)','')
ASSAM_VILLAGES.revenue_ci = ASSAM_VILLAGES.revenue_ci.str.replace(r'\n',' ')
ASSAM_VILLAGES.revenue_ci = ASSAM_VILLAGES.revenue_ci.str.strip()

ASSAM_VILLAGES.sdtname_2 = ASSAM_VILLAGES.sdtname_2.str.replace(r'\(Pt\)','')
ASSAM_VILLAGES.sdtname_2 = ASSAM_VILLAGES.sdtname_2.str.replace(r'\(Pt-I\)','')
ASSAM_VILLAGES.sdtname_2 = ASSAM_VILLAGES.sdtname_2.str.replace(r'\(Pt-II\)','')
...
for idx, row in sdrf_df.iterrows():
    ...
    if district_identifier in three_letter_distirct_identifiers_dict:
        sdrf_df.loc[idx,'expense_district_externalReference'] = three_letter_distirct_identifiers_dict[district_identifier]
...
for idx, row in sdrf_df.iterrows():
    ...
    for district in assam_districts:
        if re.findall(r'\b%s\b'%district.lower().strip(), tender_slug.lower()):
            sdrf_df.loc[idx,'expense_district_externalReference'] = district
            break
...
for idx, row in sdrf_df.iterrows():
    ...
    for revenue_circle in assam_revenue_circles:
        if re.findall(r'\b%s\b'%revenue_circle.lower().strip(), tender_slug.lower()):
            sdrf_df.loc[idx,'expense_district_externalReference'] = assam_revenue_circles_dict[revenue_circle]['district_2']
            break
...
for idx, row in sdrf_df.iterrows():
    ...
    for sub_district in assam_sub_districts:
        if re.findall(r'\b%s\b'%sub_district.lower(), tender_slug.lower()):
            sdrf_df.loc[idx,'expense_district_externalReference'] = assam_subdist_dict[sub_district]['district_2']
            break
...
for idx, row in sdrf_df.iterrows():
    ...
    for revenue_circle in assam_revenue_circles:
        if re.findall(r'\b%s\b'%revenue_circle.lower().strip(), tender_slug.lower()):
            ...
            sdrf_df.loc[idx,'tender_district_title_description'] = assam_revenue_circles_dict[revenue_circle]['district_2']
            break
...
for idx, row in sdrf_df.iterrows():
    ...
    for sub_district in assam_sub_districts:
        if re.findall(r'\b%s\b'%sub_district.lower(), tender_slug.lower()):
            sdrf_df.loc[idx,'tender_district_title_description'] = assam_subdist_dict[sub_district]['district_2']
            break
...
for idx, row in sdrf_df.iterrows():
    ...
    for revenue_circle in assam_revenue_circles:
        if re.findall(r'\b%s\b'%revenue_circle.lower().strip(), tender_slug.lower()):
            sdrf_df.loc[idx,'tender_district_location'] = assam_revenue_circles_dict[revenue_circle]['district_2']
            break
...
for idx, row in sdrf_df.iterrows():
    ...
    for sub_district in assam_sub_districts:
        if re.findall(r'\b%s\b'%sub_district.lower(), tender_slug.lower()):
            sdrf_df.loc[idx,'tender_district_location'] = assam_subdist_dict[sub_district]['district_2']
            break

# BTC FLAG
sdrf_df['BTC_flag'] = None
for idx, row in sdrf_df.iterrows():
    BTC_flag = False
    ...
    sdrf_df.loc[idx,'BTC_flag'] = BTC_flag
...
for idx, row in sdrf_df.iterrows():
    district1 = row['expense_district_externalReference']
    ...
    sdrf_df.loc[idx,'DISTRICT_FINALISED'] = DISTRICT_SELECTED

sdrf_df.to_csv(os.getcwd()+'/IDS-DRR-Assam/Sources/TENDERS/data/floodtenders_districtgeotagged.csv',index=False)

print('Total number of flood related tenders: ', sdrf_df.shape[0])
print('Number of tenders whose district could not be geo-tagged: ',sdrf_df[sdrf_df['DISTRICT_FINALISED']=='NA'].shape[0])
print('Number of tenders whose district identification is a CONFLICT: ',sdrf_df[sdrf_df['DISTRICT_FINALISED']=='CONFLICT'].shape[0])